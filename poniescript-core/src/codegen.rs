...
".to_string();
	...
	pub fn codegen(&mut self, args: &Args, ast: &Ast, output: &mut dyn std::io::Write) -> std::io::Result<()> {
		use std::io::Write;
		...
		let mut cg = Codegen::new(self.db);
